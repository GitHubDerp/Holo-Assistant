...
SYSTEMCOMMANDS = ["shutdown","status","new","delet","time","date","alarm","open"]
...
#openProgramm/file
def execute():
	inputvar = input('\033[33mProgram/Ordner/Datei\033[0m\n')
	if inputvar == "program":
...
def alarm():
	...
	if inputvar == "wecker":
		inputvar = input('-')
		start_new_thread(subAlarm,(inputvar,))
	if inputvar == "timer":
...
def Main():
	...
	if inputvar == SYSTEMCOMMANDS[6]:#Alarm
		alarm()
	if inputvar == SYSTEMCOMMANDS[7]:#Open
		execute()
